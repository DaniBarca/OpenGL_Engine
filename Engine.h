...
public:
	...
	Engine* LoadShader(string vertex_path, string fragment_path, std::map<string, string> parse_dict, int n_lights, GLuint* out_shader_id);
	Engine* LoadShader(std::vector<std::string> paths, std::vector<GLenum> types, std::map<string,string> parse_dict, int n_lights, GLuint* out_shader_id);
};
